 ...
 */
export class IdProvider implements NameProvider {
   constructor(
   ...
   findNextLocalId(type: string, proposal: string, container: AstNode): string {
   ...
   findNextGlobalId(type: string, proposal: string, packageId: string): string {
      const knownIds = this.services.shared.workspace.IndexManager.allElements(type)
         .filter(element => this.services.references.ScopeProvider.getPackageId(element) === packageId)
