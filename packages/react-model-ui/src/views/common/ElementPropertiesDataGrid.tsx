...
export function ElementPropertiesDataGrid(): React.ReactElement {
      ...
      (property: ElementPropertyRow): void => {
         if (property.name) {
            dispatch({
               ...
               property: { ...property, id: findNextUnique(toId(property.name), element.properties, attr => attr.id!) }
            });
         }
      },
   ...
   const defaultEntry = React.useMemo<Property>(
      () => ({
         ...
         name: findNextUnique('New Property', element.properties, attr => attr.name!)
      }),
      [element.properties]
   );
